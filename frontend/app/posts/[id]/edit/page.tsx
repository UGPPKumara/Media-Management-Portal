...
export default function EditPostPage({ params }: { params: { id: string } }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.put(`http://localhost:5000/api/posts/${params.id}`, formData, {
        headers: { Authorization: `Bearer ${token}` }
      });
      router.push(`/posts/${params.id}`); // Redirect back to single post view
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-2xl mx-auto">
        <Link href={`/posts/${params.id}`} className="flex items-center text-gray-600 hover:text-blue-600 mb-6 w-fit">
